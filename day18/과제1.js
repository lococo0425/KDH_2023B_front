...
const 전체 = [
     {번호 : 1 , 시간s: "4:42" ,시간e: "5:42" , 사용자: "유재석"} 
] ;
...
function 좌석출력(){
    ...
    for(let i = 1; i<=6; i++){
        ...
        if( 전체.번호.indexOf(i+"") >= 0 && 전체.시간e[ 전체.번호.indexOf(i+"")] ==''  ){

            html +=`<div class="classBox"> ${ 전체.사용자[전체.번호.indexOf(i+"")] } </div>`
        } else{
...
function 종료시간등록(){
    ...
    전체.시간e[전체.번호.indexOf(번호2)]=종료시간a;
...
function 등록함수(){        // f s
    ...
    const 전체 ={번호 : 번호2, 시간e: 종료시간a, 시간s:시작시간, 사용자:사용자2 }
...
function 출력함수(){
    ...
    const 종료PC번호 = 전체.번호.indexOf(PC번호선택값);
    // const 총금액 = 0; /* 하루매출 */
